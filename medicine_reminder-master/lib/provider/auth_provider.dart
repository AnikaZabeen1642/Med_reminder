...
class AuthProvider extends ChangeNotifier {
  ...
  // Modify the signInWithPhone method to sign in with email and password

  Future<void> registerWithEmailPassword(BuildContext context, String email, String password) async {
    try {
      // Attempt to register the user with email and password using Firebase Authentication
      ...
      if (userCredential.user != null) {
        // Save UID or perform any additional setup after registration
        _uid = userCredential.user!.uid;
        showSnackBar(context, "Registration successful!");
      }
    } on FirebaseAuthException catch (e) {
      // Handle any FirebaseAuthException errors
      showSnackBar(context, e.message.toString());
    }
  }


// Modify the verifyOtp method to match email and password from Firebase database

  void verifyEmailPassword({
    required BuildContext context,
    required String email, // email entered by the user
    required String password, // password entered by the user
    required Function onSuccess, // callback function for success
  }) async {
    _isLoading = true;
    notifyListeners();
    ...
    try {
      ...
      if (userCredential.user != null) {
        _uid = userCredential.user!.uid; // Set the UID
